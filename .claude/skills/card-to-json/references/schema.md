...
```json
{
  ...
  "eligibility_flags":  { "新戶": {}, "已登錄": {} }   /* 不寫 default = 留未選,讓使用者選 */
}
```
...
- `eligibility.flags`:二元**資格**(新戶 / 已登錄…);見下方「資格」。已取代舊的 `requires_activation`
  旗標(該旗標已退役;匯入舊檔時會自動轉成 `已登錄` 資格)。
...
// 卡層:註冊每項資格的預設狀態
"eligibility_flags": { "新戶": {}, "已登錄": {} }   // 不寫 default = 未選(慣例);要預設可寫 {"default": false|true}
```
...
- `flags`(資格,如新戶 / 已登錄):一組 **AND 前置條件**。卡層 `eligibility_flags[name].default`
...
- `reward.point_name` + 頂層 `point_programs`:回饋是**點數**時規則只給名稱;**點值(TWD/點)屬帳本**
  (`basis:"fixed"` 用官方比值、`"estimate"` 用你錨定的兌換估值;`prices[].from` 可隨時間記多筆,
  正好對應 beancount 的 price db)。記帳時用「**交易日當時生效**的點值」換算金額。
