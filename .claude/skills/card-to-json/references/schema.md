...
- `limit_pools` / `eligibility_pools`:見下方「上限(共用池)」「門檻(共用)」。
...
```json
{
  ...
  "tiers": { "mode": "flat" },
  "limits": { /* 見「上限」,可為空物件 {} */ },
  "stacking": { "layer": "base", "group": "sinopac", "select_group": "..." },
  "settlement": "once",
...
- `settlement: "once"`:里程碑(編輯器標「里程碑(達標給一次)」):完成一次發一筆、單獨列示、**完全脫離 RELATIONAL 軸**(不參與擇優、也不佔共用上限)。逐筆累積型回饋用 `"recurring"`(編輯器「逐筆回饋」)。
...
// 卡層:註冊每項資格的預設狀態
"eligibility_flags": { "新戶": {}, "活動登錄": { "cycle": "monthly" } }
// 不寫 default = 未選(慣例);要預設可寫 {"default": false|true}
// cycle = 重設週期(once 一次性 預設 / monthly / quarterly / yearly),省略 = once
```
...
- `cycle`(重設週期):`once`(一次性,預設;新戶/身分)/ `monthly` / `quarterly` / `yearly`
  (每月登錄、每季任務…需重新達成)。**「多久重設」是規則靜態事實(進卡片);「這期達成沒」是
  記帳/分析的動態真實**(分析可逐期套用、記帳依事實表判定)。例:運動達標(每月)、新戶(一次性)。
...
> **退役說明:取高(自動取當期消費最高 K 類,Citi Custom Cash 式)已移除。** 它本質是跨期累積排名,
> 屬記帳整合後才有意義,目前無卡片使用故下架。**使用者主動每月自選方案(國泰 CUBE 自選)不是取高**——
> 那是「使用者告知選哪個」,用上面的**擇一·自選(`select_group` pick)**表達。
  ...
  "cards": [
    {
      ...
      "rules": {
        ...
        "din":   { "id": "din",   "name": "餐飲 5%", "card": "示範銀行 旋鑽卡", "account": "Liabilities:CreditCard:Demo:旋鑽", "account_match": "exact", "is_active": true, "period": { "cycle": "monthly" }, "match": { "categories": ["dining"] }, "eligibility": {}, "reward": { "type": "cashback", "method": "percentage", "rate": 0.05 }, "tiers": { "mode": "flat" }, "limits": {}, "stacking": { "layer": "bonus", "group": "demo" } },
        "trv":   { "id": "trv",   "name": "旅遊 5%", "card": "示範銀行 旋鑽卡", "account": "Liabilities:CreditCard:Demo:旋鑽", "account_match": "exact", "is_active": true, "period": { "cycle": "monthly" }, "match": { "categories": ["travel"] }, "eligibility": {}, "reward": { "type": "cashback", "method": "percentage", "rate": 0.05 }, "tiers": { "mode": "flat" }, "limits": {}, "stacking": { "layer": "bonus", "group": "demo" } }
      }
    }
  ]
}
...
8. `select_group`(擇優取最高 / 自選)若適用 → 決定這筆是否被取代。
...
5. 多重上限 → `limits.caps[]`;前$X消費 → `metric:"spend"`;前N筆 → `metric:"count"`;超額累進 → `tiers.mode:"marginal"`;費率依當期計數(品牌數/天數…)→ `tiers.mode:"distinct_count"`(+`count_label`);使用者自選方案 → 擇一·自選(`select_group` pick)。
