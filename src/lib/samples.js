...
const rotateCard = {
  ...
  rules: {
    ...
    q1: { id: 'rq1', name: 'Q1 輪動 超市 5%（季上限 $1,500 · 需登錄）', account: ACCT.rotate, match: { categories: ['supermarket'] }, eligibility: { flags: ['活動登錄'] }, period: { cycle: 'quarterly', start: '2026-01-01', end: '2026-03-31' }, reward: { type: 'cashback', method: 'percentage', rate: 0.05 }, limits: { caps: [{ metric: 'spend', window: 'period', max: 1500 }] }, stacking: { layer: 'bonus' } },
    q2: { id: 'rq2', name: 'Q2 輪動 加油 5%（季上限 $1,500 · 需登錄）', account: ACCT.rotate, match: { categories: ['gas'] }, eligibility: { flags: ['活動登錄'] }, period: { cycle: 'quarterly', start: '2026-04-01', end: '2026-06-30' }, reward: { type: 'cashback', method: 'percentage', rate: 0.05 }, limits: { caps: [{ metric: 'spend', window: 'period', max: 1500 }] }, stacking: { layer: 'bonus' } },
